...
- **Diagramas visuais (pedido PERMANENTE e repetido):** todo o repositório deve ensinar com **representações visuais coloridas e ANIMADAS**, não só texto + código — em especial lições de **arquitetura, fluxo, sequência e estado**.
  - **Forma preferida (pedido do usuário, 2026-06-15):** **fluxograma de decisão com impacto** — losangos = decisões; folhas = a opção/API **e a consequência** (`impacto: …`). Não pipeline linear quando uma decisão se aplica. Ensina *por que escolher*, não só *os passos*.
  - **Técnica validada:** SVG inline; paleta via **`style="fill:var(--accent)"`** (CSS inline) — **NUNCA** `fill="var(--accent)"` (atributo de apresentação não resolve `var()`, renderiza vazio). Animação por **CSS num segundo `<style>` no corpo** (o validador só checa o PRIMEIRO `<style>`, do head, então um bloco escopado depois é seguro). Idiomas da casa: conectores com fluxo (`stroke-dashoffset`), anel pulsante na decisão (`@keyframes` opacity), partícula viajante (`transform-box:view-box`+`translateY`), hover-lift. **Sempre** `@media (prefers-reduced-motion)` + `@media print` desligando a animação. **Sem** renderizadores JS (quebraria o `<script>` byte-idêntico).
  - Prévia para aprovação visual: `docs/previews/*.html` (chat não renderiza animação CSS — abrir no navegador). Pilar "Visual teaching" do `CLAUDE.md` + `.claude/rules/lesson-authoring.md`. A **arquitetura de referência** macro (componentes/integrações) e micro (fluxo seguro: tools, agents, skills, guardrails) é a Lição 22 (capstone).
